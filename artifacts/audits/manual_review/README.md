...
Copy `gold_template.json` to `gold.json`. For each function, fill `dfg_edges` (an empty list is valid), `span_alignment_ok`, and `reviewer_ids`, using only the matching worksheet. Then run `manual_extraction_audit.py import` and `evaluate`.
